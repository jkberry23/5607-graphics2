...
void read_input_file(const char *input_filepath)
{
	...
	while(fscanf(infile, "%s", keyword) != EOF) 
	{
		...
		else if(strcmp(keyword, "bkgcolor") == 0)
		{
			got_bkgcolor++;
			fgets(line, 100, infile);
			if(sscanf(line, "%lf %lf %lf %lf", &f1, &f2, &f3, &f4) < 3)
			{
		...
		else if(strcmp(keyword, "mtlcolor") == 0)
		{
			params.nummtlcolors++;
			fgets(line, 100, infile);
			if(sscanf(line, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf", &f1, &f2, &f3, &f4, &f5, &f6, &f7, &f8, &f9, &i1, &f10, &f11) < 12)
			{
			...
			if(f10 < 0 || f10 > 1)
			{
				printf("line %d: mtlcolor opacity value must be in the range [0,1]\n", curline);
				exit(1);
			}
			if(f11 <= 0)
			{
				printf("line %d: mtlcolor index of refraction must be > 0\n", curline);
			}
		}
	...
	while(fscanf(infile, "%s", keyword) != EOF) 
	{
		if(strcmp(keyword, "mtlcolor") == 0)
		{
			idx_cur_color++;
			fgets(line, 100, infile);
			sscanf(line, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf", &f1, &f2, &f3, &f4, &f5, &f6, &f7, &f8, &f9, &i1, &f10, &f11);
			...
			params.mtlcolors[idx_cur_color].opacity = f10;
			params.mtlcolors[idx_cur_color].ior = f11;
		}
	...
	if(0)
	{
		...
		for(int i = 0; i < params.nummtlcolors; i++)
		{
			...
			printf("\topacity: %lf\n", params.mtlcolors[i].opacity);
			printf("\tior: %lf\n", params.mtlcolors[i].ior);
		}
